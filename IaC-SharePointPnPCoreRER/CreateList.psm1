...
function New-RequestList {
    ...
    if (! $List) {
        $List = New-PnPList -Title $ListTitle -Url "lists/ExtProjectRequest" -Template GenericList -EnableVersioning -OnQuickLaunch:$false
...
function New-MailList {
    ...
    $ListTitle = "Ext Project Request EMails"
    ...
    $List = Get-PnPList -Identity $ListTitle -ErrorAction SilentlyContinue
    if (! $List) {
        $List = New-PnPList -Title $ListTitle -Url "lists/ExtProjectStatusEmails" -Template GenericList -EnableVersioning -OnQuickLaunch:$false
        $UpdatedList = Set-PnPList -Identity $ListTitle -Description $ListDescription 
    }
    # Creating fields if does not exist
